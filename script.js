...
function initNavigation() {
    ...
    navItems.forEach(item => {
        item.addEventListener("click", (e) => {
            ...
            // 6. AUTO-SCROLL KE KONTEN (khusus tampilan mobile/HP)
            // Hanya aktif jika lebar layar ≤ 768px (tampilan HP/tablet)
            if (window.innerWidth <= 768) {
                const targetSection = document.getElementById(targetSectionId);
                if (targetSection) {
                    // Tunggu sebentar agar section selesai ditampilkan dulu, baru scroll
                    setTimeout(() => {
                        targetSection.scrollIntoView({
                            behavior: 'smooth', // Animasi scroll mulus
                            block: 'start'      // Posisi konten di bagian atas layar
                        });
                    }, 100); // Delay 100ms agar menu sempat tertutup sebelum scroll
                }
            }
        });
    });
...
/**
 * FUNGSI 3: Toggle Hamburger Menu (Mobile Navigation)
 * Menampilkan atau menyembunyikan menu navigasi saat tombol hamburger diklik di HP.
 */
function initHamburgerMenu() {
    const hamburger = document.getElementById('hamburger-menu');
    const navMenu = document.getElementById('nav-menu');

    if (!hamburger || !navMenu) return; // Keluar jika elemen tidak ditemukan

    // Toggle menu buka/tutup saat hamburger diklik
